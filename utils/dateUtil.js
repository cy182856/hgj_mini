...
/**yyyyMMdd格式的日期计算间隔天 */
const getTwoDateDiff = (startDate,endDate) =>{
  var oDate1, oDate2, iDays;
  oDate1 = new Date(startDate.substr(0,4), startDate.substr(4,2) - 1, startDate.substr(6,2));
  oDate2 = new Date(endDate.substr(0,4), endDate.substr(4,2) - 1, endDate.substr(6,2));
  iDays = parseInt(Math.abs(oDate1 - oDate2) / 1000 / 60 / 60 / 24);
  return startDate >= endDate ? iDays : -iDays;
}
...
//为日期加上指定单位 返回yyyy/mm/dd/ HH:MM:SS
const dateAddDesUnit = (date,unit,num) =>{
  if(!date || date ==''){
    return '';
  }
  if(!unit ||unit == '' || !num){
...
//yyyyMMdd 格式日期加上指定单位并返回
const dateStrAddUtils = (strDate, unit, num) =>{
  if(!strDate || !unit || !num
    || strDate.length != 8){
    return;
  }
  ...
  return dateAddMonth(new Date(year,month-1,day),unit,num);
}
...
module.exports = {
  ...
  dateAddDesUnit:dateAddDesUnit,
  getMonthLength:getMonthLength,
  dateStrAddUtils: dateStrAddUtils,
  getTwoDateDiff: getTwoDateDiff
}