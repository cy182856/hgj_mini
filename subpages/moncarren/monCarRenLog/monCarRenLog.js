...
Page({
  ...
  queryCarRenewLogs:function(type){
    ...
    that.showLoading(!0)
    app.req.postRequest(api.queryMonCarRenLog, queryParams).then(function (value) {
      console.log("queryMonCarRenLog 返回", value);
  ...
  makeInvoiceCheck:function(e){
    ...
    app.req.postRequest(api.monCarInvoiceCheck,data).then(res=>{
      if(res.data.respCode == '000'){
          wx.navigateTo({
            url: '/subpages/moncarren/monCarInvoice/monCarInvoice?orderId=' + orderId,
          })       
      }else{
        wx.showToast({
          icon:'none',
