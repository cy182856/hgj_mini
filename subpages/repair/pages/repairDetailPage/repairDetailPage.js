...
// 取消报修
  ...
  pingjia(event){
    ...
    let msgBody = this.data.msgBody;
    app.req.postRequest(api.evaluateRepairOrder,
      {repairDate:repairDate,repairSeqId:repairSeqId,rateScore:rateScore,msgBody:msgBody} ).then(res => {
        var data = res.data;
        this.showLoading(0);
  ...
  onLoad: function (options) {
    ...
    app.req.postRequest(api.repairQuery,d).then(res=>{
      console.log('报修详情详细数据:',res);
      var respData = res.data;
      that.showLoading(0);
      if(respData.respCode == '000'){
        //var obj = respData.repairDtos[0];
        var obj = respData.repairLogList[0];
        var index = '0';
        // 评分
        var repairScore = '';
        if(obj.repairStatus != 'WOSta_Visit'){
          repairScore = that.data.technicianAssessStar
        }else{
          repairScore = obj.repairScore;
          index = obj.satisFaction;
        }
        // 满意度
        ...
        that.setData({
          ...
          //comObj:respData,
          fileList:fileList,
          technicianAssessStar:repairScore,
          repairMsgBody:repairMsg,
  ...
  submitRepairMsg() {
    ...
    let repairNum = this.data.repairNum;
    ...
    data['repairMsg'] = msgBody;
    data['repairScore'] = that.data.technicianAssessStar;
    data['satisFaction'] = that.data.satisFaction;
    data['cstCode'] = app.storage.getCstCode();
    data['wxOpenId'] = app.storage.getWxOpenId();
    data['proNum'] = app.storage.getProNum();
    app.req.postRequest(api.repairAddMsgBody,data).then(res=>{
      console.log('res====>', res);
      if (res.data.respCode === '000') {
        Toast.alert({message: '评价成功'}).then(res=>{
          that.setData({msgList:[],repairMsgBody:''});
          that.onOpen();
          wx.redirectTo({
            url: '../repairDetailPage/repairDetailPage?repairNum='+repairNum,
          })
        })
  ...
  handleChange(e) {
    let satisFaction = e.detail.value;
    this.setData({
