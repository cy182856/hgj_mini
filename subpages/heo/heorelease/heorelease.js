...
Page({
  ...
  onLoad: function (options) {
    app.loading(),that = this, this.showLoading(!1),heo.heoinfo.init();
    heo.heoinfo.initOwnerHeoTypes(options.theme);
    wx.getSavedFileList({
      success (res) {
  ...
  onConfirmCheckNorG:function(e){
    ...
    if(e.detail.value.needOrGive != 'N' && e.detail.value.needOrGive != 'G'){
      for(var j in that.data.ownerHeoTypes){
        if(that.data.ownerHeoTypes[j].heoType == '99'){
          that.setData({
            heoTypeIndex:j,
            heoType:'99',
            heoTypeDisable:true
  ...
  onChangeHouseIsPub:function(e){
    this.setData({ 
      checked: e.detail ,
      houseIsPub : e.detail ? 'Y' :'N'
    });
  },
  //文件上传
  afterRead(event) {
